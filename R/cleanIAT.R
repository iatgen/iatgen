...
#' \code{num.negative.removed} is the grand total number of trials scored as missing because they recorded a negative (impossible) reaction time.
#' \code{num.negative.removed.prac1} is the number of such trials in the first practice block.
#' \code{num.negative.removed.crit1} is the number of such trials in the first critical block.
#' \code{num.negative.removed.prac2} is the number of such trials in the second practice block.
#' \code{num.negative.removed.crit2} is the number of such trials in the second critical block.
#' \code{negative.rate} is the proportion of all trials that were scored as missing for this reason.
...
cleanIAT <- function(prac1, crit1, prac2, crit2, timeout.drop = TRUE, timeout.ms = 10000, fasttrial.drop = FALSE, fasttrial.ms = 400, fastprt.drop = TRUE, fastprt.percent = .10, fastprt.ms = 300, error.penalty = FALSE, error.penalty.ms = 600, inclusive.sd = TRUE) {
  ...
  for (i in 1:nrow(raw.prac1)) {
    for (j in 1:ncol(raw.prac1)) {
      if (!is.na(raw.prac1[i, j])) {
        raw.prac1[i, j] <- add.leading.zeros(raw.prac1[i, j])
      }
    }
  }
  ...
  for (i in 1:nrow(raw.prac2)) {
    for (j in 1:ncol(raw.prac2)) {
      if (!is.na(raw.prac2[i, j])) {
        raw.prac2[i, j] <- add.leading.zeros(raw.prac2[i, j])
      }
    }
  }
  ...
  num.negative.removed.prac1 <- count.negative(clean.latencies.prac1)
  num.negative.removed.crit1 <- count.negative(clean.latencies.crit1)
  num.negative.removed.prac2 <- count.negative(clean.latencies.prac2)
  ...
  temp.drop <- drop.negative(clean.latencies.prac1, clean.correct.prac1, clean.stim.number.prac1)
  clean.latencies.prac1 <- temp.drop$latencies
  clean.correct.prac1 <- temp.drop$correct
  clean.stim.number.prac1 <- temp.drop$stim.number
  ...
  temp.drop <- drop.negative(clean.latencies.prac2, clean.correct.prac2, clean.stim.number.prac2)
  clean.latencies.prac2 <- temp.drop$latencies
  clean.correct.prac2 <- temp.drop$correct
  clean.stim.number.prac2 <- temp.drop$stim.number
  ...
  num.negative.removed <- sum(c(
    num.negative.removed.prac1, num.negative.removed.crit1,
    num.negative.removed.prac2, num.negative.removed.crit2
  ), na.rm = TRUE)
  ...
  # SUM total for final reporting
  num.fasttrial.removed <- sum(c(
    num.fasttrial.removed.prac1, num.fasttrial.removed.crit1,
    num.fasttrial.removed.prac2, num.fasttrial.removed.crit2
  ), na.rm = T)
  ...
  negative.rate <- num.negative.removed / sum(num.raw.trials.prac1, num.raw.trials.crit1, num.raw.trials.prac2, num.raw.trials.crit2, na.rm = T)
  ...
  for (i in 1:nrow(clean.correct.latencies.prac1)) {
    for (j in 1:ncol(clean.correct.latencies.prac1)) {
      if (!is.na(clean.correct.latencies.prac1[i, j])) { # cannot have NA in comparisons
        if (raw.correct.prac1[i, j] == "X") {
          clean.correct.latencies.prac1[i, j] <- NA
        }
      }
    }
  }
  ...
  for (i in 1:nrow(clean.correct.latencies.prac2)) {
    for (j in 1:ncol(clean.correct.latencies.prac2)) {
      if (!is.na(clean.correct.latencies.prac2[i, j])) { # cannot have NA in comparisons
        if (raw.correct.prac2[i, j] == "X") {
          clean.correct.latencies.prac2[i, j] <- NA
        }
      }
    }
  }
    ...
    for (j in 1:ncol(clean.latencies.prac1)) {
      if (!is.na(raw.correct.prac1[i, j])) {
        if (error.penalty == TRUE && is.numeric(error.penalty.ms)) {
          if (raw.correct.prac1[i, j] == "X" && !is.na(clean.latencies.prac1[i, j])) {
            clean.latencies.prac1[i, j] <- clean.correct.means.prac1[i] + error.penalty.ms
          }
        } else if (error.penalty == TRUE && error.penalty.ms == "2SD") {
          if (raw.correct.prac1[i, j] == "X" && !is.na(clean.latencies.prac1[i, j])) {
            clean.latencies.prac1[i, j] <- clean.correct.means.prac1[i] + 2 * clean.std.correct.prac1[i]
          }
        } else if (error.penalty == FALSE) {
          if (raw.correct.prac1[i, j] == "X" && !is.na(clean.latencies.prac1[i, j])) {
            clean.latencies.prac1[i, j] <- clean.latencies.prac1[i, j]
          }
        }
        if (raw.correct.prac1[i, j] == "C") {
          clean.latencies.prac1[i, j] <- clean.correct.latencies.prac1[i, j]
        }
      }
      if (is.na(raw.correct.prac1[i, j])) {
        clean.latencies.prac1[i, j] <- NA
  ...
  for (i in 1:nrow(clean.latencies.prac2)) {
    for (j in 1:ncol(clean.latencies.prac2)) {
      if (!is.na(raw.correct.prac2[i, j])) {
        if (error.penalty == TRUE && is.numeric(error.penalty.ms)) {
          if (raw.correct.prac2[i, j] == "X" && !is.na(clean.latencies.prac2[i, j])) {
            clean.latencies.prac2[i, j] <- clean.correct.means.prac2[i] + error.penalty.ms
          }
        } else if (error.penalty == TRUE && error.penalty.ms == "2SD") {
          if (raw.correct.prac2[i, j] == "X" && !is.na(clean.latencies.prac2[i, j])) {
            clean.latencies.prac2[i, j] <- clean.correct.means.prac2[i] + 2 * clean.std.correct.prac2[i]
          }
        } else if (error.penalty == FALSE) {
          if (raw.correct.prac2[i, j] == "X" && !is.na(clean.latencies.prac2[i, j])) {
            clean.latencies.prac2[i, j] <- clean.latencies.prac2[i, j]
          }
        }
        if (raw.correct.prac2[i, j] == "C") {
          clean.latencies.prac2[i, j] <- clean.correct.latencies.prac2[i, j]
        }
      }
      if (is.na(raw.correct.prac2[i, j])) {
        clean.latencies.prac2[i, j] <- NA
  ...
  return(list(
    ...
    num.negative.removed.prac1 = num.negative.removed.prac1,
    num.negative.removed.crit1 = num.negative.removed.crit1,
    num.negative.removed.prac2 = num.negative.removed.prac2,
